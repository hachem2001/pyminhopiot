...
from piconetwork.flooder import NodeLP_fastflood_mod, NodeLP_flood, NodeLP_flood_mod
import sys
import nographs
...
NodeLP_Jitter_Configuration.JITTER_MIN_VALUE = 8*0.6
NodeLP_Jitter_Configuration.JITTER_MAX_VALUE = (8*0.6)*(NodeLP_Jitter_Configuration.JITTER_INTERVALS+1)
...
NodeLP_fastflood_mod.NODE_RECEPTION_OF_PACKET_DURATION = NodeLP_fastflood_mod.NODE_RECEPTION_OF_PACKET_DURATION = NodeLP.NODE_RECEPTION_OF_PACKET_DURATION

SOURCE_RECURRENT_TRANSMISSIONS_DELAY = NodeLP_Jitter_Configuration.JITTER_MAX_VALUE * 20
SIMULATION_TOTAL_DURATION = SOURCE_RECURRENT_TRANSMISSIONS_DELAY * 20
HEARING_RADIUS = 30.0
DENSITY_RADIUS = 15.0
...
node_classes = [NodeLP_fastflood_mod] # [NodeLP_flood_mod, NodeLP_fastflood_mod, NodeLP_mod]
node_classes_names = ["FASTFLOODING"] # ["FLOODING", "FASTFLOODING", "PROTOCOL"]
suppression_modes = [[None]] # [[None], [None], [NodeLP_Suppression_Mode.REGULAR, NodeLP_Suppression_Mode.CONSERVATIVE, NodeLP_Suppression_Mode.AGGRESSIVE, NodeLP_Suppression_Mode.BOLD]]
label_names = ["FASTFLOODING"] # ["FLOODING", "FASTFLOODING"] + ["REGULAR", "CONSERVATIVE", "AGGRESSIVE", "BOLD"]
...
for node_class_index in range(len(node_classes)):
    ...
    for suppression_mode in suppression_modes[node_class_index]:
        ...
        for node in nodes:
            if isinstance(node, NodeLP_mod) or isinstance(node, NodeLP_flood_mod) or isinstance(node, NodeLP_fastflood_mod):
